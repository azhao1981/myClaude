...
4. **逐个击破**: 不要一次性写所有测试用例。可以列计划，但**一次只写一个测试**，通过后再写下一个。
5. **Setup/Teardown 禁写**: 禁止在 `setUp`/`tearDown`、`beforeEach`/`afterEach`、`up`/`down` 等钩子中执行写入或删除数据的操作。所有数据变更必须在**测试用例内部**显式完成，确保副作用可追溯。
