...
4. **真实链路优先，Mock 按需**:
   - **所有依赖默认走真实链路**：TDD 是打通所有环节的过程，无论是内部 DB/Redis/MQ 还是外部 API，都应优先调用真实实现。
   - **Mock 只在以下情况使用**：用户**明确要求** Mock，或测试标注为 **unit test** 时。
   - **Mock 例外**：仅当外部系统不可控（会产生真实费用、无法访问）时，才按需 Mock。
5. **零后门操作 (No Backdoor Access)**: 测试代码**严禁**直接执行 SQL (DELETE/UPDATE/INSERT) 来准备或清理数据。所有数据变更必须通过**业务功能代码**完成。
6. **先验证根因，再修复 (Verify Before Fix)**: 修复失败测试前，必须先追踪实际数据流（数据库查询、日志分析）确认根因，**严禁**通过篡改测试数据来凑通过。
...
* **外部依赖**: 默认调用真实实现。只有用户明确要求或标注为 unit test 时才使用 Mock/Stub。
...
3. **真实链路优先**: 所有依赖默认走真实链路；Mock 只在用户明确要求或 unit test 时使用。
4. **测试无特权**: 测试代码禁止直接执行 DML（DELETE/UPDATE/INSERT）或 DDL（CREATE/DROP），包括 setUp/tearDown 钩子中。
5. **遇阻即停**: 环境连接问题或 Schema 变更需求，必须停下来与用户交互。
6. **逐个击破**: 一次只写一个测试，只运行一个测试，通过后再写下一个。
7. **Red-Green-Refactor**: 每个测试用例必须走完完整的三步循环，不可跳过 Refactor。
